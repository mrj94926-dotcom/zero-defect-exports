...
function applyFilters(keepPage = false) {
    productState.filtered = productState.products.filter(p => {
        const matchCategory = productState.category === 'all' || p.category === productState.category;
        const matchPrice = p.price <= productState.maxPrice;
...
function renderPagedProducts() {
    ...
    if (pageItems.length === 0) {
        ...
        let badges = '';
        if (p.isBestSeller) {
            badges += '<div style="position: absolute; top: 10px; right: 10px; background: #FF9800; color: white; padding: 4px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; z-index: 10; box-shadow: 0 2px 5px rgba(0,0,0,0.2);"><i class="fas fa-star" style="margin-right: 3px;"></i>Best Seller</div>';
        }
        if (p.isNew) {
            badges += '<div style="position: absolute; top: 10px; left: 10px; background: #4CAF50; color: white; padding: 4px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; z-index: 10; box-shadow: 0 2px 5px rgba(0,0,0,0.2);"><i class="fas fa-sparkles" style="margin-right: 3px;"></i>New</div>';
        }
        return `
...
function quickViewProduct(id) {
    ...
    if (product) {
        ...
        if (product.stock > 0) {
            ...
            document.getElementById('qv-add-to-cart').disabled = true;
        }
...
function filterProductsByPrice(value) {
    ...
    productState.maxPrice = parseInt(value);
    applyFilters();
}
...
function handleCheckout(e) {
    ...
    cart.forEach(cartItem => {
        ...
        if (productIndex !== -1) {
            if ((products[productIndex].stock || 0) >= cartItem.quantity) {
                ...
                inventoryIssue = true;
                showSuccessMessage(`Insufficient stock for ${cartItem.name}. Available: ${products[productIndex].stock || 0}`, 'error');
            }
        }
    });

    if (inventoryIssue) return;
    ...
    addNotification('order', 'New Order Placed', `Order #${orderId} by ${checkoutData.name}`);
